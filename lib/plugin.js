module.exports = function (mongoose) {
  ...
  function createParams(model, docs, paths, options, lean) {
    ...
    paths.forEach(function (path) {
      var noOfSubpaths = path.path.split('.').length
  ...
  function populate(opts, level, cb) {
    var docs = opts.docs,
      paths = opts.paths,
      lvlPaths = paths.filter(function (path) {
        return (path.path.split('.').length - 1) === level
      }),
    ...
    while (path = lvlPaths.shift()) {
      ...
      if (model != null) {
        populateOpts = extend({}, opts.options[path.path], {
          path: path.path,
          model: model
        })

        if (opts.lean) {
          populateOpts.options = extend({
            lean: true
  ...
  function getModelFromPath(db, model, path) {
    ...
    path.path.split('.').forEach(function (subpath, index) {
      currentPath = {
        path: (currentPath ? (currentPath.path + '.') : '') + subpath,
        model: path.model ? path.model : null
      };

      schema = currentPath.model ? currentPath.model.schema : null;
      ...
      // if model is found for current path, extract model and schema
      if (currentPath.model) {
        candidateModel = currentPath.model
  ...
  function deconstruct(paths) {
    ...
    paths.forEach(function (path) {
      ...
      path.path.split('.').forEach(function (subpath, index) {
        currentPath = {
          path: (currentPath ? (currentPath.path + '.') : '') + subpath.trim(),
